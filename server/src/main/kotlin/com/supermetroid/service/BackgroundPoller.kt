...
class BackgroundPoller(
    ...
    suspend fun start() {
        ...
        pollingJob = CoroutineScope(Dispatchers.Default).launch {
            println("🚀 Background poller started (interval: ${updateInterval}ms)")
            pollLoop()
        }
    }
    ...
    private suspend fun pollLoop() {
        ...
        while (currentCoroutineContext().isActive) {
            try {
                ...
                println("Poll completed in ${pollDuration}ms")
                
                // Sleep until next poll
                val sleepTime = maxOf(0, updateInterval - pollDuration)
                delay(sleepTime)
    ...
    suspend fun resetCache() {
        mutex.withLock {
            ...
            println("🔄 All caches reset - fresh game state will be read on next poll")
        }
    }
} 