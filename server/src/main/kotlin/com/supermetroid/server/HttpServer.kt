...
class HttpServer(
    ...
    private fun Application.configureServer() {
        ...
        install(CORS) {
            ...
            anyHost() // Allow from any host for development
        }
    }
    ...
    private fun Application.configureRouting() {
        routing {
            ...
            get("/api/status") {
                val cachedState = poller.getCachedState()
                call.respond(cachedState)
            }
            ...
            get("/api/reset-cache") {
                try {
                    poller.resetCache()
                    val response = mapOf("message" to "All caches reset - fresh game state will be read on next poll")
                    call.respond(response)
                } catch (e: Exception) {
                    call.respond(
                        HttpStatusCode.InternalServerError,
