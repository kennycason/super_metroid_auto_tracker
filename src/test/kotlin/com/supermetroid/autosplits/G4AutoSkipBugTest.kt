...
class G4AutoSkipBugTest {
    ...
    fun `test isConditionAlreadyMet for G4 specifically`() {
        // Import the RoomIds constant
        val statuesRoomId = 42602 // RoomIds.STATUES value (0xA66A)

        // Test case 1: All bosses defeated AND in statues room (should be true)
        val gameStateG4Complete = GameState(
            gameState = 8, // NORMAL_GAMEPLAY - required for processing
            roomId = statuesRoomId, // In statues room
            bosses = Bosses(
                kraid = true,
                phantoon = true,
                draygon = true,
        ...
        // Test case 2: All bosses defeated but NOT in statues room (should be false)
        val gameStateG4BossesOnlyComplete = GameState(
            gameState = 8, // NORMAL_GAMEPLAY - required for processing
            roomId = 12345, // Some other room
            bosses = Bosses(
                kraid = true,
                phantoon = true,
                draygon = true,
        ...
        // Test case 3: In statues room but missing bosses (should be false)
        ...
        assertFalse(engine.isConditionAlreadyMet(g4Split, gameStateG4BossesOnlyComplete),
